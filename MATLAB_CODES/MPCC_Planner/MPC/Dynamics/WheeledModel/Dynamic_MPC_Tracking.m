...
D=3;                     % half of wheel-to-wheel distance 
...
q_dot = [x_dot; y_dot; theta_dot; phi1_dot; phi2_dot; gamma_dot];       % system states
...
T = [0;0;0;tau;0;omega];           % system control inputs
Input = [tau;omega];
n_controls = 2;
...
%Df = diag([0,0,0,Fn,Fn])*q_dot;
B= mx*theta_dot^2*[cos(theta);sin(theta);0;0;0;0];

C=[1 0 D*sin(theta) -cos(theta+gamma)*ro 0  0;...
   0 1 -D*cos(theta) -sin(theta+gamma)*ro 0  0;...
   1 0 0 0 -cos(theta)*ro  0;...
   0 1 0 0 -sin(theta)*ro  0];

Cdot=[0 0 D*theta_dot*cos(theta) sin(theta+gamma)*ro*(theta_dot+gamma_dot) 0  0;...
      0 0 D*theta_dot*sin(theta) -cos(theta+gamma)*ro*(theta_dot+gamma_dot) 0  0;...
      0 0 0 0 sin(theta)*theta_dot*ro   0;...
      0 0 0 0 -cos(theta)*theta_dot*ro  0];
...
args.lbx(6:n_states:n_states*(N+1),1) = -2*pi; %state gamma lower bound
args.ubx(6:n_states:n_states*(N+1),1) = 2*pi;  %state gamma upper bound

args.lbx(7:n_states:n_states*(N+1),1) = -2; %state xdot lower bound
args.ubx(7:n_states:n_states*(N+1),1) = v_max;  %state xdot upper bound

args.lbx(8:n_states:n_states*(N+1),1) = -2; %state ydot lower bound
args.ubx(8:n_states:n_states*(N+1),1) = v_max;  %state ydot upper bound
...
args.lbx(12:n_states:n_states*(N+1),1) = -0.2; %state gamma_dot lower bound
args.ubx(12:n_states:n_states*(N+1),1) = 0.2;  %state gamma_dot upper bound
...
args.lbx(n_states*(N+1)+2:n_controls:n_states*(N+1)+n_controls*N,1) = -0.2; %omega lower bound
args.ubx(n_states*(N+1)+2:n_controls:n_states*(N+1)+n_controls*N,1) = 0.2; %omega upper bound
...
t0 = 0;
x0 = [0 ; 0 ; pi/2 ; 0; 0 ; 0 ; 0 ; 0; 0; 0; 0; 0];                              % initial state.
...
t_mpc = zeros(sim_tim / dt,1);                          % Store times needed to solve the problem
...
Xout = Generate_Trajectory(D,Tvec);
while(mpciter < length(Xout)-N) 
    ...
    for k = 1:N                                         % Update the values of the reference trajecotry
        args.p(k*(n_states)+1:k*(n_states)+12) = [Xout(k+mpciter,1), Xout(k+mpciter,2), Xout(k+mpciter,3), Xout(k+mpciter,4), 0, 0, 0, 0, 0, 0, 0, 0];
    end
    ...
    t(mpciter+1) = t0;
    [t0, x0, u0] = Dynamic_shift(dt, t0, x0, u,f);           % Apply control input and get the next initial state from the dynamics
...
Draw_MPC_tracking (t,x_hist,predicted_x,u_cl,Xout,N,rob_diam, obs_diam,obs_x, obs_y)