...
D=1;                     % half of wheel-to-wheel distance 
...
Q = diag([5, 5, 5]);
Q_Terminal = diag([20,20, 2, 0, 0.0, 0.0, 0, 0]);                    % weighing matrices (states)
...
for k = 1:N
    ...
    k1 = f(st,con);                   % get the dynamics expressed in the states st
    k2 = f(st+dt/2*k1,con);
    k3 = f(st+dt/2*k2,con);
    k4 = f(st+dt*k3,con);
    next_st = dt/6*(k1+2*k2+2*k3+k4);  
    
    x_d = st(5:length(st)-1) + next_st(1:3);         
    st_next_euler(5:length(st_next_euler)-1) = x_d(1:3);
    st_next_euler(length(st_next_euler)) = k1(4);

    x = st(1:3) + (dt*x_d);
    st_next_euler(1:3)= x;
    st_next_euler(4) = st(4) + next_st(4);
...
args.lbx(8:n_states:n_states*(N+1),1) = -0.3; %state gamma_dot lower bound
args.ubx(8:n_states:n_states*(N+1),1) = 0.3;  %state gamma_dot upper bound
...
args.lbx(n_states*(N+1)+2:n_controls:n_states*(N+1)+n_controls*N,1) = -0.3; %omega lower bound
args.ubx(n_states*(N+1)+2:n_controls:n_states*(N+1)+n_controls*N,1) = 0.3; %omega upper bound
...
t0 = 0;
x0 = [0 ; 0 ; pi/2 ; 0; 0 ; 0 ; 0 ; 0];                              % initial state.
...
while(mpciter < length(Xout)-N) 
    ...
    for k = 1:N                                         % Update the values of the reference trajecotry
        args.p(k*(n_states)+1:k*(n_states)+8) = [Xout(k+mpciter,1), Xout(k+mpciter,2), Xout(k+mpciter,3), 0, 0, 0, 0, 0];
    end
