...
Q = diag([50, 50, 5]);                                                    % weighing matrices (states)
...
[Xout, ref, wayps] = Generate_Trajectory(1,Tvec);

while(i < length(Xout)-N) 
    ...
    for k = 1:N                                         % Update the values of the reference trajecotry
        ...
        args.p(k*(n_states)+1:k*(n_states)+4) = [Xout(k+i,1), Xout(k+i,2), 0, 0];
    end
...
Draw_MPC_tracking_kin (t,x_hist,predicted_x,u_cl,Xout,N,rob_diam, obs_diam, obs_x, obs_y, t_mpc, wayps)
