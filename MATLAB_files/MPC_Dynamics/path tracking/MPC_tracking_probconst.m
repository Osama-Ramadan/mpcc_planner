...
D=1.2;                     % half of wheel-to-wheel distance 
...
for k = 1:N
    ...
    g = [g;st(5)*sin(st(3)+st(4))-st(6)*cos(st(3)+st(4))+D*cos(st(4))*st(7)];
end
...
args.lbg(n_states*(N+1)+1 :n_states*(N+1)+(N)) = -0.0;  % -1e-20  % Equality constraints
args.ubg(n_states*(N+1)+1 :n_states*(N+1)+(N)) = 0.0;  % 1e-20   % Equality constraints
