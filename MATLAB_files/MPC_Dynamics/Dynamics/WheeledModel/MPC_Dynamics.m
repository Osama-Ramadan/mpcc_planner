...
C=[1 0 D*sin(theta) -cos(theta+gamma)*ro 0  0;...
   ...
   1 0 0 0 -cos(theta)*ro1  0;...
   0 1 0 0 -sin(theta)*ro1  0];
...
Cdot=[0 0 D*theta_dot*cos(theta) sin(theta+gamma)*ro*(theta_dot+gamma_dot) 0  0;...
      ...
      0 0 0 0 sin(theta)*theta_dot*ro1   0;...
      0 0 0 0 -cos(theta)*theta_dot*ro1  0];
...
args.lbx(12:n_states:n_states*(N+1),1) = -0.9; %state gamma_dot lower bound
args.ubx(12:n_states:n_states*(N+1),1) = 0.9;  %state gamma_dot upper bound
...
args.lbx(n_states*(N+1)+2:n_controls:n_states*(N+1)+n_controls*N,1) = -0.9; %omega lower bound
args.ubx(n_states*(N+1)+2:n_controls:n_states*(N+1)+n_controls*N,1) = 0.9; %omega upper bound
%----------------------------------------------
t0 = 0;
...
Draw_MPC_PS_Obstacles (t,x_hist,predicted_x,u_cl,xs,N,rob_diam,obs_x,obs_y,obs_diam,obst_pose)