function Omni_Draw_MPC_PS_Obstacles (t,xx,xx1,u_cl,xs,N,rob_diam,obs_x,obs_y,obs_diam,mpc_costs, obst_pose)
...
for k = 1:(size(xx,1)-1)
    ...
    plot(xx1(count:count+N+1,1),xx1(count:count+N+1,2),'r--*');
    %for j = 2:N+1
     %   plot(xx1(count+j,1)+xp,xx1(count+j,2)+yp,'--r'); % plot robot circle
    %end
