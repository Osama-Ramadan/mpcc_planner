...
l = 3;                    % the distance between the steering wheel and the body point

v_max = 2.1;                % Input constraints on both linear speed and steering speed
v_min = -v_max;
omega_max = pi/4; 
omega_min = -omega_max;
...
P = SX.sym('P',n_states + N*(n_states+n_controls));                             % parameters (which include at the initial state of the robot and the reference state)
...
Q = diag([1, 10, 0.1, 0.1]);                                                    % weighing matrices (states)
R = diag([0.5,0.05]);                                                           % weighing matrices (controls)
...
for k = 1:N
    ...
    obj = obj+(st-P(k*(n_states+2)-1:k*(n_states+2)+2))'*Q*(st-P(k*(n_states+2)-1:k*(n_states+2)+2)) + ...
              (con-P(k*(n_states+2)+3:k*(n_states+2)+4))'*R*(con-P(k*(n_states+2)+3:k*(n_states+2)+4)) ; % calculate obj
...
Tinit = 0;
Tfinal = sim_tim;
Tvec=(Tinit : T : 2*Tfinal);
circleode = @(~,x)circle_trajectory(l,v_ref,omega_ref,x);
...
while(mpciter < sim_tim / T) 
    ...
    for k = 1:N                                         % Update the values of the reference trajecotry
        args.p(k*(n_states+2)-1:k*(n_states+2)+2) = [Xout(k+mpciter,1), Xout(k+mpciter,2), Xout(k+mpciter,3), Xout(k+mpciter,4)];
...
Draw_MPC_tracking (t,x_hist,predicted_x,u_cl,Xout,N,rob_diam)
