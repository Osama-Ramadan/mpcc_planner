...
class DW_COSTMAP(object):
    ...
    def get_costmap_x_y(self, world_x, world_y):                     
        ...
        costmap_x = int(np.round((world_x - self.origin.position.x) / self.resolution))
        costmap_y = int(np.round((abs(world_y - self.origin.position.y)) / self.resolution))
