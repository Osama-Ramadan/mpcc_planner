...
#sudo apt-get install python3-scipy python3-matplotlib ipython3 python3-pandas python3-sympy 
...
from mpc_igoneo.goals_planner import PATH_PLANNER
import time
...
class MPC_CONTROLLER(Node):
    ...
    def timer_callback(self):
        ...
        #self.MPC_run(state_update)
        goals = [[float(0),float(0)],[float(1),float(1)], [float(2), float(4)], [float(3), float(9)], [float(5), float(10)]]
        lookahead = 3
        self.currentx = self.currentx+self.dx
        self.currenty = self.currenty+0.5*self.dx
        self.current_pose = [self.currentx ,self.currenty]
        accuracy_parameter = 50
        
        self.current_waypoints = self.get_next_waypoints(lookahead, goals, self.current_path, self.current_waypoints,self.current_pose)
        response = self.send_waypoints(self.current_waypoints, accuracy_parameter)
        self.current_path = np.dstack([response.localpath_x,response.localpath_y])[0]

        print('Current waypoints : ', self.current_waypoints)
        print('------------------------')
    ...
    def Init_MPC(self):
        ...
        P = SX.sym('P',self.n_states + self.n_states)                                       # parameters (which include at the initial state of the robot and the reference state)
        ...
        Q = diag([20, 20, 0.01, 0, 0, 0, 0, 0]);                                                    # weighing matrices (states)
        Q_terminal = diag([200,200, 1, 0, 0.0, 0, 0, 0]);                                                      # weighing matrices (states)
        ...
        for k in range(self.N):
            if k!= self.N-1:                                        
                ...
                obj = obj+(st-P[self.n_states:2*self.n_states]).T@Q_terminal@(st-P[self.n_states:2*self.n_states]) + con.T@R@con
        ...
        goal1 = (np.array([0, 0, 0 , 0, 0, 0, 0, 0])).reshape(8,1)
        goal2 = (np.array([5, -2, 0 , 0, 0, 0, 0, 0])).reshape(8,1)
        goal3 = (np.array([10, -7, 0 , 0, 0, 0, 0, 0])).reshape(8,1)
        goal4 = (np.array([15, 0, pi/3 , 0, 0, 0, 0, 0])).reshape(8,1)
        goal5 = (np.array([16, 8.5, pi/2 , 0, 0, 0, 0, 0])).reshape(8,1)
        goal6 = (np.array([10, 16, pi , 0, 0, 0, 0, 0])).reshape(8,1)
        goal7 = (np.array([2, 9, pi , 0, 0, 0, 0, 0])).reshape(8,1)
        self.goals = [goal1, goal2, goal3, goal4, goal5, goal6, goal7]
        self.goal_counter = 0
        self.xs = self.goals[self.goal_counter]
    ...
    def MPC_run(self, feedback_state):
        ...
        final_state = reshape(sol[self.n_states*(2):self.n_states*(3)],self.n_states,1)
        ...
        current_goal = self.goals[self.goal_counter]
        goal_dist = np.sqrt((current_goal[0]-state_opt[self.N-1,0])**2 + (current_goal[1]-state_opt[self.N-1,1])**2)
        if(goal_dist < 0.5 and self.goal_counter < len(self.goals)-1):
